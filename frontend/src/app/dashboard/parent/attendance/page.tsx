...
export default function ParentAttendancePage() {
  ...
  const loadAttendance = async () => {
    try {
      const res = await parentsApi.getChildAttendance(selectedChildId);
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm">Track your child&apos;s attendance</p>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-heading font-semibold text-gray-900 flex items-center gap-2">
            <HiClipboardCheck className="w-5 h-5 text-primary-600" /> Calendar
          </h2>
          <input
            type="month"
            value={currentMonth}
