...
import { messagesApi, teachersApi } from '@/lib/api';
...
export default function TeacherMessagesPage() {
  ...
  const loadData = async () => {
    try {
      ...
      try {
        const classesRes = await teachersApi.getMyClasses();
        const students = (classesRes.data.data || []).flatMap((c: any) => c.students || []);
  ...
  const inboxMessages = messages.filter((m) => activeTab === 'inbox' ? true : false);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm">Communicate with students, parents, and staff</p>
        </div>
