...
function uploadValueImage($file, $existing = null) {
    if ($file['error'] === UPLOAD_ERR_NO_FILE) return $existing;
    $ext = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
    $allowed = ['jpg','jpeg','png','gif','webp'];
    if (!in_array($ext, $allowed)) return false;
    $name = 'value_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_value'])) {
    ...
    $image = $idV > 0 ? (mysqli_fetch_assoc(mysqli_query($conn, "SELECT image FROM `tbl_core_values` WHERE id=$idV"))['image'] ?? null) : null;
    $newImage = uploadValueImage($_FILES['image'], $image);
    if ($newImage === false && $_FILES['image']['error'] !== UPLOAD_ERR_NO_FILE) { $error = 'Invalid image file.'; }
        ...
        if ($idV > 0) {
            mysqli_query($conn, "UPDATE `tbl_core_values` SET `title`='$title',`description`='$desc',`icon`='$icon',`image`=$img,`sort_order`=$order,`is_active`=$active WHERE id=$idV");
            $msg = 'Core value updated.';
        } else {
            mysqli_query($conn, "INSERT INTO `tbl_core_values` (`title`,`description`,`icon`,`image`,`sort_order`,`is_active`) VALUES ('$title','$desc','$icon',$img,$order,$active)");
            $msg = 'Core value added.';
        }
        if (!empty($conn->error)) $error = $conn->error;
    }
    logActivity($conn, $idV > 0 ? 'update_value' : 'add_value', $title);
}

if (isset($_GET['delete'])) {
    $idD = (int)$_GET['delete'];
    $r = mysqli_fetch_assoc(mysqli_query($conn, "SELECT image FROM `tbl_core_values` WHERE id=$idD"));
    if ($r && $r['image'] && file_exists(dirname(__FILE__) . '/../' . $r['image'])) unlink(dirname(__FILE__) . '/../' . $r['image']);
    mysqli_query($conn, "DELETE FROM `tbl_core_values` WHERE id=$idD");
    $msg = 'Core value deleted.';
    logActivity($conn, 'delete_value', "Deleted value ID $idD");
}
if (isset($_GET['toggle'])) {
    $idT = (int)$_GET['toggle'];
    $r = mysqli_fetch_assoc(mysqli_query($conn, "SELECT is_active FROM `tbl_core_values` WHERE id=$idT"));
    $new = $r['is_active'] ? 0 : 1;
    mysqli_query($conn, "UPDATE `tbl_core_values` SET `is_active`=$new WHERE id=$idT");
    $msg = 'Status toggled.';
}

$values = mysqli_query($conn, "SELECT * FROM `tbl_core_values` ORDER BY sort_order ASC");
$editRow = null;
if (isset($_GET['edit'])) {
    $eid = (int)$_GET['edit'];
    $editRow = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM `tbl_core_values` WHERE id=$eid"));
}
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <main id="main" class="main">
    <div style="padding:24px;">
      <div class="page-header-card">
        <div>
          <h1><i class="bi bi-heart"></i> Manage Core Values</h1>
          <p>Add, edit, or manage core values displayed on the homepage About section.</p>
        </div>
      ...
      <div class="content-card">
        <div class="card-header"><i class="bi bi-plus-circle"></i> <?php echo $editRow ? 'Edit' : 'Add'; ?> Core Value</div>
        <div class="card-body">
          <form method="POST" enctype="multipart/form-data" class="row g-3">
            ...
            <div class="col-md-3">
              ...
              <input type="text" name="icon" class="form-control-custom" value="<?php echo htmlspecialchars($editRow['icon'] ?? 'bi bi-heart'); ?>">
            </div>
            <div class="col-md-1">
            ...
            <div class="col-md-2 d-flex align-items-end">
              <div class="form-check">
                <input type="checkbox" name="is_active" class="form-check-input" id="va" <?php echo (!isset($editRow) || $editRow['is_active']) ? 'checked' : ''; ?>>
                <label class="form-check-label" for="va">Active</label>
              </div>
            </div>
            <div class="col-md-4">
            ...
            <div class="col-12">
              <button type="submit" name="save_value" class="btn-save"><i class="bi bi-save"></i> <?php echo $editRow ? 'Update' : 'Add'; ?> Value</button>
              <?php if ($editRow): ?><a href="manage-values.php" class="btn btn-secondary ms-2"><i class="bi bi-x"></i> Cancel</a><?php endif; ?>
            </div>
          </form>
        </div>
      </div>
      ...
      <div class="content-card mt-4">
        <div class="card-header"><i class="bi bi-list"></i> Core Values</div>
        <div class="card-body p-0">
          <div class="table-responsive">
            <table class="table table-hover mb-0">
              <thead><tr><th>Image</th><th>#</th><th>Title</th><th>Active</th><th style="width:150px">Actions</th></tr></thead>
              <tbody>
                <?php if (mysqli_num_rows($values) == 0): ?>
                  <tr><td colspan="5" class="text-center text-muted py-4">No core values yet.</td></tr>
                <?php else: ?>
                  <?php while ($v = mysqli_fetch_assoc($values)): ?>
                    <tr>
                      <td><?php if ($v['image']): ?><img src="../<?php echo htmlspecialchars($v['image']); ?>" class="img-preview"><?php else: ?><span class="text-muted">—</span><?php endif; ?></td>
                      <td><?php echo $v['sort_order']; ?></td>
                      <td><strong><?php echo htmlspecialchars($v['title']); ?></strong><br><small class="text-muted"><?php echo htmlspecialchars(substr($v['description'], 0, 80)); ?></small></td>
                      <td><?php echo $v['is_active'] ? '<span class="badge bg-success">Active</span>' : '<span class="badge bg-secondary">Hidden</span>'; ?></td>
                      <td>
                        <a href="?edit=<?php echo $v['id']; ?>" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil"></i></a>
                        <a href="?toggle=<?php echo $v['id']; ?>" class="btn btn-sm btn-outline-warning"><i class="bi bi-eye<?php echo $v['is_active'] ? '-slash' : ''; ?>"></i></a>
                        <a href="?delete=<?php echo $v['id']; ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Delete?')"><i class="bi bi-trash"></i></a>
                      </td>
                    </tr>
                  <?php endwhile; ?>
                <?php endif; ?>
