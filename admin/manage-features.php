...
function uploadFeatureImage($file, $existing = null) {
    if ($file['error'] === UPLOAD_ERR_NO_FILE) return $existing;
    $ext = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
    $allowed = ['jpg','jpeg','png','gif','webp'];
    if (!in_array($ext, $allowed)) return false;
    $name = 'feature_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_feature'])) {
    ...
    $image = $idF > 0 ? (mysqli_fetch_assoc(mysqli_query($conn, "SELECT image FROM `tbl_features` WHERE id=$idF"))['image'] ?? null) : null;
    $newImage = uploadFeatureImage($_FILES['image'], $image);
    if ($newImage === false && $_FILES['image']['error'] !== UPLOAD_ERR_NO_FILE) { $error = 'Invalid image file.'; }
        ...
        if ($idF > 0) {
            mysqli_query($conn, "UPDATE `tbl_features` SET `title`='$title',`description`='$desc',`icon`='$icon',`image`=$img,`sort_order`=$order,`is_active`=$active WHERE id=$idF");
            $msg = 'Feature updated.';
        } else {
            mysqli_query($conn, "INSERT INTO `tbl_features` (`title`,`description`,`icon`,`image`,`sort_order`,`is_active`) VALUES ('$title','$desc','$icon',$img,$order,$active)");
            $msg = 'Feature added.';
        }
        if (!empty($conn->error)) $error = $conn->error;
    }
    logActivity($conn, $idF > 0 ? 'update_feature' : 'add_feature', $title);
}

if (isset($_GET['delete'])) {
    $idD = (int)$_GET['delete'];
    $r = mysqli_fetch_assoc(mysqli_query($conn, "SELECT image FROM `tbl_features` WHERE id=$idD"));
    if ($r && $r['image'] && file_exists(dirname(__FILE__) . '/../' . $r['image'])) unlink(dirname(__FILE__) . '/../' . $r['image']);
    mysqli_query($conn, "DELETE FROM `tbl_features` WHERE id=$idD");
    $msg = 'Feature deleted.';
    logActivity($conn, 'delete_feature', "Deleted feature ID $idD");
}
if (isset($_GET['toggle'])) {
    $idT = (int)$_GET['toggle'];
    $r = mysqli_fetch_assoc(mysqli_query($conn, "SELECT is_active FROM `tbl_features` WHERE id=$idT"));
    $new = $r['is_active'] ? 0 : 1;
    mysqli_query($conn, "UPDATE `tbl_features` SET `is_active`=$new WHERE id=$idT");
    $msg = 'Status toggled.';
}

$features = mysqli_query($conn, "SELECT * FROM `tbl_features` ORDER BY sort_order ASC");
$editRow = null;
if (isset($_GET['edit'])) {
    $eid = (int)$_GET['edit'];
    $editRow = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM `tbl_features` WHERE id=$eid"));
}
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <style>
    ...
    .img-preview { width: 48px; height: 48px; object-fit: cover; border-radius: 8px; border: 2px solid #f0f0f0; }
    ...
    <div style="padding:24px;">
      <div class="page-header-card">
        <div>
          ...
          <p>Add, edit, or manage "Why Choose Us" feature cards displayed on the homepage.</p>
        </div>
      ...
      <div class="content-card">
        <div class="card-header"><i class="bi bi-plus-circle"></i> <?php echo $editRow ? 'Edit' : 'Add'; ?> Feature</div>
        <div class="card-body">
          <form method="POST" enctype="multipart/form-data" class="row g-3">
            <input type="hidden" name="id" value="<?php echo $editRow['id'] ?? 0; ?>">
            <div class="col-md-6">
              <label class="form-label-custom">Title *</label>
              <input type="text" name="title" class="form-control-custom" value="<?php echo htmlspecialchars($editRow['title'] ?? ''); ?>" required>
            </div>
            <div class="col-md-3">
              <label class="form-label-custom">Icon (Bootstrap class)</label>
              <input type="text" name="icon" class="form-control-custom" value="<?php echo htmlspecialchars($editRow['icon'] ?? 'bi bi-star'); ?>">
            </div>
            <div class="col-md-1">
            ...
            <div class="col-md-2 d-flex align-items-end">
              <div class="form-check">
                <input type="checkbox" name="is_active" class="form-check-input" id="fea" <?php echo (!isset($editRow) || $editRow['is_active']) ? 'checked' : ''; ?>>
                <label class="form-check-label" for="fea">Active</label>
              </div>
            </div>
            <div class="col-md-4">
              ...
              <?php if (!empty($editRow['image'])): ?>
                <div class="existing-img-label"><img src="../<?php echo htmlspecialchars($editRow['image']); ?>" class="img-preview"> Existing</div>
              <?php endif; ?>
            </div>
            <div class="col-12">
              <label class="form-label-custom">Description</label>
              <textarea name="description" class="form-control-custom" rows="2"><?php echo htmlspecialchars($editRow['description'] ?? ''); ?></textarea>
            </div>
            <div class="col-12">
              <button type="submit" name="save_feature" class="btn-save"><i class="bi bi-save"></i> <?php echo $editRow ? 'Update' : 'Add'; ?> Feature</button>
              <?php if ($editRow): ?><a href="manage-features.php" class="btn btn-secondary ms-2"><i class="bi bi-x"></i> Cancel</a><?php endif; ?>
            </div>
          </form>
        </div>
      </div>
      ...
      <div class="content-card mt-4">
        <div class="card-header"><i class="bi bi-list"></i> Features</div>
        <div class="card-body p-0">
          <div class="table-responsive">
            <table class="table table-hover mb-0">
              <thead><tr><th>Image</th><th>#</th><th>Title</th><th>Icon</th><th>Active</th><th style="width:150px">Actions</th></tr></thead>
              <tbody>
                <?php if (mysqli_num_rows($features) == 0): ?>
                  <tr><td colspan="6" class="text-center text-muted py-4">No features yet.</td></tr>
                <?php else: ?>
                  <?php while ($f = mysqli_fetch_assoc($features)): ?>
                    <tr>
                      <td><?php if ($f['image']): ?><img src="../<?php echo htmlspecialchars($f['image']); ?>" class="img-preview"><?php else: ?><span class="text-muted">—</span><?php endif; ?></td>
                      <td><?php echo $f['sort_order']; ?></td>
                      <td><strong><?php echo htmlspecialchars($f['title']); ?></strong><br><small class="text-muted"><?php echo htmlspecialchars(substr($f['description'], 0, 80)); ?></small></td>
                      <td><i class="<?php echo htmlspecialchars($f['icon']); ?>"></i></td>
                      <td><?php echo $f['is_active'] ? '<span class="badge bg-success">Active</span>' : '<span class="badge bg-secondary">Hidden</span>'; ?></td>
                      <td>
                        <a href="?edit=<?php echo $f['id']; ?>" class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil"></i></a>
                        <a href="?toggle=<?php echo $f['id']; ?>" class="btn btn-sm btn-outline-warning"><i class="bi bi-eye<?php echo $f['is_active'] ? '-slash' : ''; ?>"></i></a>
                        <a href="?delete=<?php echo $f['id']; ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Delete?')"><i class="bi bi-trash"></i></a>
                      </td>
                    </tr>
                  <?php endwhile; ?>
                <?php endif; ?>
