...
if(empty($imgfile)){
  ...
  // $id = $_GET['updateid'];
  // $query = "SELECT ImageUrl FROM  `tbl_gallery_post`  WHERE id='$id' ";
  ...
  // $ImageUrl = $img['ImageUrl'];
            ...
            $query=mysqli_query($conn,"update tbl_gallery_post set CategoryNameId = '$GalleryCategory' where id='$postid'");
            if($query)
            {
                // header('location:update-blogpost.php?'&msg=Blog Image Updated Successfully');
                header("location:manage-gallerypost.php?msg=Gallery Image Updated Successfully");
            }
            else{
...
$GalleryCategory = $_POST['GalleryCategory'];
$postid=intval($_GET['updateid']);
$query=mysqli_query($conn,"update tbl_gallery_post set CategoryNameId = '$GalleryCategory', ImageUrl='$imgnewfile' where id='$postid'");
if($query)
{
    // header('location:update-blogpost.php?'&msg=Blog Image Updated Successfully');
    header("location:manage-gallerypost.php?msg=Gallery Image Updated Successfully");
...
<!--adding gallery post starting-->
                ...
                <form name="addpost" novalidate  enctype="multipart/form-data" method="post">
   ...
   <div class="row">
      ...
      <?php
            ...
            tbl_school_category.id = tbl_gallery_post.CategoryNameId WHERE tbl_gallery_post.id = '$postid'";
      ...
      <input type="text" class="form-modern" id="posttitle" value="<?php echo htmlentities($row['category']);?>" name="posttitle"  readonly>
      </div>
    ...
    <div class="col-12 position-relative">
            ...
            <select class="form-modern" id="validationTooltip04" required  name="GalleryCategory">
              ...
              <option selected  value="<?php echo $row['postid'] ?>"><?php echo $row['category'] ?></option>
              ...
              <?php
                ...
                while($call=mysqli_fetch_array($query_run)){
      ...
      <img src="Gallery Images/<?php echo $row['ImageUrl'] ?>" width="300"/>
      <br /><br>
      </div>
      </div>
      </div>
      ...
      <div class="text-center">
          <div class="d-grid gap-2 mt-3">
            <button class="btn-modern btn-modern-primary" name="update" type="submit">Update  Image </button>
          </div>
