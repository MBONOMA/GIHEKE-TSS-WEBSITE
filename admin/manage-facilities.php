...
function uploadFacilityImage($file, $existing = null) {
    if ($file['error'] === UPLOAD_ERR_NO_FILE) return $existing;
    $ext = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
    $allowed = ['jpg','jpeg','png','gif','webp'];
    if (!in_array($ext, $allowed)) return false;
    $name = 'facility_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['save_facility'])) {
    ...
    $image = $idF > 0 ? (mysqli_fetch_assoc(mysqli_query($conn, "SELECT image FROM `tbl_facilities` WHERE id=$idF"))['image'] ?? null) : null;
    $newImage = uploadFacilityImage($_FILES['image'], $image);
    if ($newImage === false && $_FILES['image']['error'] !== UPLOAD_ERR_NO_FILE) { $error = 'Invalid image file.'; }
        ...
        if ($idF > 0) {
            mysqli_query($conn, "UPDATE `tbl_facilities` SET `name`='$name',`description`='$desc',`image`=$img,`sort_order`=$order,`is_active`=$active WHERE id=$idF");
            $msg = 'Facility updated.';
        } else {
            mysqli_query($conn, "INSERT INTO `tbl_facilities` (`name`,`description`,`image`,`sort_order`,`is_active`) VALUES ('$name','$desc',$img,$order,$active)");
            $msg = 'Facility added.';
        }
        if (!empty($conn->error)) $error = $conn->error;
    }
    logActivity($conn, $idF > 0 ? 'update_facility' : 'add_facility', $name);
}

if (isset($_GET['delete'])) {
    $idD = (int)$_GET['delete'];
    $r = mysqli_fetch_assoc(mysqli_query($conn, "SELECT image FROM `tbl_facilities` WHERE id=$idD"));
    if ($r && $r['image'] && file_exists(dirname(__FILE__) . '/../' . $r['image'])) unlink(dirname(__FILE__) . '/../' . $r['image']);
    mysqli_query($conn, "DELETE FROM `tbl_facilities` WHERE id=$idD");
    $msg = 'Facility deleted.';
    logActivity($conn, 'delete_facility', "Deleted facility ID $idD");
}
if (isset($_GET['toggle'])) {
    $idT = (int)$_GET['toggle'];
    $r = mysqli_fetch_assoc(mysqli_query($conn, "SELECT is_active FROM `tbl_facilities` WHERE id=$idT"));
    $new = $r['is_active'] ? 0 : 1;
    mysqli_query($conn, "UPDATE `tbl_facilities` SET `is_active`=$new WHERE id=$idT");
    $msg = 'Status toggled.';
}

$facilities = mysqli_query($conn, "SELECT * FROM `tbl_facilities` ORDER BY sort_order ASC");
$editRow = null;
if (isset($_GET['edit'])) {
    $eid = (int)$_GET['edit'];
    $editRow = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM `tbl_facilities` WHERE id=$eid"));
}
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <main id="main" class="main">
    <div style="padding:24px;">
      <div class="page-header-card">
        <div>
          <h1><i class="bi bi-building"></i> Manage Facilities</h1>
          <p>Add, edit, or manage school facilities displayed on the homepage.</p>
        </div>
      ...
      <div class="content-card">
        <div class="card-header"><i class="bi bi-plus-circle"></i> <?php echo $editRow ? 'Edit' : 'Add'; ?> Facility</div>
        <div class="card-body">
          <form method="POST" enctype="multipart/form-data" class="row g-3">
            <input type="hidden" name="id" value="<?php echo $editRow['id'] ?? 0; ?>">
            <div class="col-md-6">
              <label class="form-label-custom">Facility Name *</label>
            ...
            <div class="col-md-2 d-flex align-items-end">
              <div class="form-check">
                <input type="checkbox" name="is_active" class="form-check-input" id="fa" <?php echo (!isset($editRow) || $editRow['is_active']) ? 'checked' : ''; ?>>
                <label class="form-check-label" for="fa">Active</label>
              </div>
            </div>
            <div class="col-md-4">
              ...
              <?php if (!empty($editRow['image'])): ?>
                <div class="existing-img-label"><img src="../<?php echo htmlspecialchars($editRow['image']); ?>" class="img-preview"> Existing image</div>
              <?php endif; ?>
            </div>
            <div class="col-12">
            ...
            <div class="col-12">
              <button type="submit" name="save_facility" class="btn-save"><i class="bi bi-save"></i> <?php echo $editRow ? 'Update' : 'Add'; ?> Facility</button>
              <?php if ($editRow): ?><a href="manage-facilities.php" class="btn btn-secondary ms-2"><i class="bi bi-x"></i> Cancel</a><?php endif; ?>
            </div>
          </form>
        </div>
      </div>
      ...
      <div class="content-card mt-4">
        <div class="card-header"><i class="bi bi-list"></i> Facilities</div>
        <div class="card-body p-0">
          <div class="table-responsive">
            <table class="table table-hover mb-0">
              <thead><tr><th>Image</th><th>Order</th><th>Facility</th><th>Active</th><th style="width:150px">Actions</th></tr></thead>
              <tbody>
                <?php if (mysqli_num_rows($facilities) == 0): ?>
                  <tr><td colspan="5" class="text-center text-muted py-4">No facilities yet.</td></tr>
                <?php else: ?>
                  <?php while ($f = mysqli_fetch_assoc($facilities)): ?>
                    <tr>
                      ...
                      <td><strong><?php echo htmlspecialchars($f['name']); ?></strong><br><small class="text-muted"><?php echo htmlspecialchars(substr($f['description'], 0, 80)); ?>...</small></td>
                      <td><?php echo $f['is_active'] ? '<span class="badge bg-success">Active</span>' : '<span class="badge bg-secondary">Hidden</span>'; ?></td>
                      <td>
                        <a href="?edit=<?php echo $f['id']; ?>" class="btn btn-sm btn-outline-primary" title="Edit"><i class="bi bi-pencil"></i></a>
                        <a href="?toggle=<?php echo $f['id']; ?>" class="btn btn-sm btn-outline-warning" title="Toggle"><i class="bi bi-eye<?php echo $f['is_active'] ? '-slash' : ''; ?>"></i></a>
                        <a href="?delete=<?php echo $f['id']; ?>" class="btn btn-sm btn-outline-danger" title="Delete" onclick="return confirm('Delete this facility?')"><i class="bi bi-trash"></i></a>
                      </td>
                    </tr>
                  <?php endwhile; ?>
                <?php endif; ?>
